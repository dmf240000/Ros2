...
import { Euler as Euler2, MathUtils as MathUtils6, Vector3 as Vector32 } from "three";
...
var DataHelper = class extends AbstractService {
  ...
  fovToZoomLevel(fov) {
    ...
    return MathUtils6.clamp(temp - 2 * (temp - 50), 0, 100);
  }
  /**
   ...
   */
  vFovToHFov(vFov) {
    return MathUtils6.radToDeg(2 * Math.atan(Math.tan(MathUtils6.degToRad(vFov) / 2) * this.state.aspect));
  }
  /**
   * Converts horizontal FOV to vertical FOV
   */
  hFovToVFov(hFov) {
    return MathUtils6.radToDeg(2 * Math.atan(Math.tan(MathUtils6.degToRad(hFov) / 2) / this.state.aspect));
  }
  /**
   * @internal
   */
  ...
  cleanPanoramaPose(panoData) {
    return {
      pan: MathUtils6.degToRad(panoData?.poseHeading || 0),
      tilt: MathUtils6.degToRad(panoData?.posePitch || 0),
      roll: MathUtils6.degToRad(panoData?.poseRoll || 0)
    };
  }
  /**
...
var EventsHandler = class extends AbstractService {
  ...
  __doMove(clientX, clientY) {
    if (this.step.is(Step.CLICK) && this.__moveThresholdReached(clientX, clientY)) {
      ...
      const rotation = {
        yaw: this.config.moveSpeed * (x / this.state.size.width) * MathUtils7.degToRad(this.state.hFov),
        pitch: this.config.moveSpeed * (y / this.state.size.height) * MathUtils7.degToRad(this.state.vFov)
      };
...
var ViewerDynamics = class extends AbstractService {
  ...
  updateSpeeds() {
    ...
    this.position.setSpeed(MathUtils8.degToRad(this.config.moveSpeed * 50));
    this.roll.setSpeed(MathUtils8.degToRad(this.config.moveSpeed * 50));
  }
  /**
   * @internal
   */
